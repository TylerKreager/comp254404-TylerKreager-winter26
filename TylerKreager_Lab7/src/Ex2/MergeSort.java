...
class MergeSort {
  ...
  public static <K> void mergeSortBottomUpQueues(Queue<K> orig, Comparator<K> comp) {
    Queue<Queue<K>> lq = new LinkedQueue<>();


    while (!orig.isEmpty()) {
      Queue<K> item = new LinkedQueue<>();
      item.enqueue(orig.dequeue());
      lq.enqueue(item);
    }

    while (lq.size() > 1) {
      Queue<K> que1 = lq.dequeue();
      Queue<K> que2 = lq.dequeue();
      Queue<K> merged = new LinkedQueue<>();
      merge(que1, que2, merged, comp);
      lq.enqueue(merged);
    }

    Queue<K> sortedQue = lq.dequeue();
    while (!sortedQue.isEmpty()){
      orig.enqueue(sortedQue.dequeue());
    }
  }
  public static void main(String[] args) throws FileNotFoundException {
    ...
    //Use the implementation of Merge-Sort with Queues to sort a queue of String objects
      ...
      try (Scanner scanner = new Scanner(new File("TylerKreager_Lab7/src/Ex2/pokemonNames.txt"))) {
      ...
      mergeSortBottomUpQueues(stringQueue, strComp);
